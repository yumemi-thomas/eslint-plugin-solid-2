...
const createRule$22 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$21 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$20 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$19 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$18 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$17 = ESLintUtils.RuleCreator.withoutDocs;
var no_array_handlers_default = createRule$17({
	...
	create(context) {
		const resolveValue = (node) => {
			...
			const definition = ASTUtils.findVariable(context.sourceCode.getScope(traced), traced)?.defs[0];
...
const createRule$16 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$15 = ESLintUtils.RuleCreator.withoutDocs;
const getName = (node) => {
	switch (node.type) {
...
const createRule$14 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$13 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$12 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$11 = ESLintUtils.RuleCreator.withoutDocs;
...
const createRule$10 = ESLintUtils.RuleCreator.withoutDocs;
	...
	create(context) {
		...
		return {
			...
			VariableDeclarator(node) {
				...
				const variable = ASTUtils.findVariable(sourceCode.getScope(accessorIdentifier), accessorIdentifier);
			...
			VariableDeclarator(node) {
				...
				if (strictScopes.length > 0 && (node.id.type === "ObjectPattern" || node.id.type === "ArrayPattern") && node.init != null) {
					if (node.init.type === "Identifier") {
						...
						const variable = ASTUtils.findVariable(sourceCode.getScope(node.init), node.init.callee);
...
const kebabCase = kebabCaseModule;
